...
func (this *H2H_FriendProc) AddFriend(args *rpc_common.H2H_AddFriend, reply *rpc_common.H2H_AddFriendResult) error {
	...
	// 已有申请
	res := p.db.FriendReqs.CheckAndAdd(args.FromPlayerId, args.FromPlayerName)
	if res < 0 {
...
func (this *H2H_FosterProc) SetCat2Friend(args *rpc_common.H2H_FosterCat2Friend, result *rpc_common.H2H_FosterCat2FriendResult) error {
	...
	res := friend.add_friend_cat_in_foster(args.FromPlayerId, args.FromPlayerLevel, args.FromPlayerName, args.FromPlayerHead, args.FromPlayerCatId, args.FromPlayerCatTableId, args.FromPlayerCatLevel, args.FromPlayerCatStar)
	if res < 0 {
	...
	//result.ToFriendId = args.ToFriendId
...
func (this *H2H_FosterProc) GetCatOnFriend(args *rpc_common.H2H_FosterGetCatInfoOnFriend, result *rpc_common.H2H_FosterGetCatInfoOnFriendResult) error {
	...
	result.ToFriendLevel = player.db.Info.GetLvl()
	result.ToFriendName = player.db.GetName()
	result.ToFriendHead = player.db.Info.GetIcon()
...
func (this *H2H_FosterProc) GetPlayerFosterData(args *rpc_common.H2H_FosterGetPlayerFosterData, result *rpc_common.H2H_FosterGetPlayerFosterDataResult) error {
	...
	for i := 0; i < len(friend_player_ids); i++ {
		...
		result.PlayerFriendCats[i].PlayerName, _ = player.db.FosterFriendCats.GetPlayerName(friend_player_ids[i])
		result.PlayerFriendCats[i].PlayerHead, _ = player.db.FosterFriendCats.GetPlayerHead(friend_player_ids[i])
		result.PlayerFriendCats[i].PlayerLevel, _ = player.db.FosterFriendCats.GetPlayerLevel(friend_player_ids[i])
	}
...
func (this *H2H_FosterProc) GetEmptySlotFriendInfo(args *rpc_common.H2H_FosterGetEmptySlotFriendInfo, result *rpc_common.H2H_FosterGetEmptySlotFriendInfoResult) error {
	...
	result.FromPlayerId = args.FromPlayerId
	result.ToPlayerId = args.ToPlayerId
	result.ToPlayerName = player.db.GetName()
	result.ToPlayerHead = player.db.Info.GetIcon()
	result.ToPlayerLevel = player.db.Info.GetLvl()
...
func (this *H2H_GlobalProc) Anouncement(args *rpc_common.H2H_Anouncement, result *rpc_common.H2H_AnouncementResult) error {
	...
	if !anouncement_mgr.PushNew(args.MsgType, true, args.FromPlayerId, args.FromPlayerName, args.FromPlayerLevel, args.MsgParam1, args.MsgParam2, args.MsgParam3, args.MsgText) {
...
func (this *R2H_RanklistProc) AnouncementFirstRank(args *rpc_common.R2H_RanklistPlayerFirstRank, result *rpc_common.R2H_RanklistPlayerFirstRankResult) error {
	...
	if !anouncement_mgr.PushNew(ANOUNCEMENT_TYPE_RANKING_LIST_FIRST_RANK, false, args.PlayerId, args.PlayerName, args.PlayerLevel, args.RankType, args.RankParam, 0, "") {
		err_str := fmt.Sprintf("@@@ R2H_RanklistProc::AnouncementFirstRank Push Player[%v,%v,%v] first rank in ranklist[%v] failed", args.PlayerId, args.PlayerName, args.PlayerLevel, args.RankType)
		return errors.New(err_str)
	}
	log.Debug("@@@ R2H_RanklistProc::AnouncementFirstRank Pushed Player[%v,%v,%v] is first rank in ranklist[%v]", args.PlayerId, args.PlayerName, args.PlayerLevel, args.RankType)
	return nil
}
