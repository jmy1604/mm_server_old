...
func _make_formula_building_msg_data() *msg_client_message.MakingFormulaBuildingInfo {
	...
	d.FormulaId = proto.Int32(0)
	d.RemainSeconds = proto.Int32(0)
	d.SlotId = proto.Int32(0)
	return d
}
...
func (this *Player) get_formula_buildings() (slot_buildings []*msg_client_message.MakingFormulaBuildingInfo, maked_buildings []int32) {
	...
	for i := 0; i < len(formula_buildings); i++ {
		...
		if j == len(making_list) {
			slot_buildings[dn].SlotId = proto.Int32(formula_buildings[i])
			dn -= 1
		}
	}
	...
	if making_list != nil && len(making_list) > 0 {
		...
		for ; n < len(making_list); n++ {
			...
			slot_buildings[dn].SlotId = proto.Int32(making_list[n])
			maked_buildings = append(maked_buildings, f.BuildID)
			dn -= 1

			this.TaskUpdate(table_config.TASK_FINISH_MAKING_FORMULA_NUM, false, f.Rarity, 1)
			this.TaskUpdate(table_config.TASK_FINISH_MAKING_FORMULA_BUILDING_NUM, false, f.Rarity, 1)
		}
		...
		for i := n; i < len(making_list); i++ {
			...
			slot_buildings[i-n].SlotId = proto.Int32(making_list[i])
			slot_buildings[i-n].FormulaId = proto.Int32(fid)
			if i == n {
				slot_buildings[i-n].RemainSeconds = proto.Int32(left_seconds)
			} else {
				slot_buildings[i-n].RemainSeconds = proto.Int32(f.Time * 60)
			}
		}
...
func (this *Player) exchange_formula(formula_id int32) int32 {
	...
	response.FormulaId = proto.Int32(formula_id)
	this.Send(response)
...
func (this *Player) make_formula_building(formula_id /*, slot_id*/ int32) int32 {
	...
	for i := 0; i < len(f.CostItems); i++ {
		...
		if !o || num < need_num {
			if item_id == table_config.ITEM_MATERIAL_ID_BOARD {
				log.Error("玩家[%v]木板数量[%v]不足需要[%v]，打造失败", this.Id, num, need_num)
			} else if item_id == table_config.ITEM_MATERIAL_ID_BRICK {
				log.Error("玩家[%v]砖块数量[%v]不足需要[%v]，打造失败", this.Id, num, need_num)
			} else if item_id == table_config.ITEM_MATERIAL_ID_IRON {
				log.Error("玩家[%v]生铁数量[%v]不足需要[%v]，打造失败", this.Id, num, need_num)
			} else if item_id == table_config.ITEM_MATERIAL_ID_GOLD {
				log.Error("玩家[%v]金块数量[%v]不足需要[%v]，打造失败", this.Id, num, need_num)
			} else if item_id == table_config.ITEM_MATERIAL_ID_LEAVES {
				log.Error("玩家[%v]叶子数量[%v]不足需要[%v]，打造失败", this.Id, num, need_num)
			} else if item_id == table_config.ITEM_MATERIAL_ID_CLOTH {
				log.Error("玩家[%v]布料数量[%v]不足需要[%v]，打造失败", this.Id, num, need_num)
			} else if item_id == table_config.ITEM_MATERIAL_ID_RUBBER {
				log.Error("玩家[%v]橡胶数量[%v]不足需要[%v]，打造失败", this.Id, num, need_num)
			} else if item_id == table_config.ITEM_MATERIAL_ID_PAINT {
	...
	// update task
	this.TaskUpdate(table_config.TASK_FINISH_MAKING_FORMULA_NUM, false, 0, 1)
	this.TaskUpdate(table_config.TASK_FINISH_MAKING_FORMULA_BUILDING_NUM, false, f.Rarity, 1)
...
func (this *Player) buy_new_making_building_slot() int32 {
	...
	if this.GetDiamond() < global_id.FormulaAddNewSlotCostDiamond_51 {
	...
	this.SubDiamond(global_id.FormulaAddNewSlotCostDiamond_51, "buy_new_slot", "formula")

	response := &msg_client_message.S2CBuyMakeBuildingSlotResult{}
	response.SlotId = proto.Int32(new_slot_id)
	this.Send(response)
...
func (this *Player) speedup_making_building( /*slot_id int32*/ ) int32 {
	...
	if slot_buildings[0].GetSlotId() > 0 && slot_buildings[0].GetFormulaId() > 0 {
		...
		this.TaskUpdate(table_config.TASK_FINISH_MAKING_FORMULA_NUM, false, formula.Rarity, 1)
		this.TaskUpdate(table_config.TASK_FINISH_MAKING_FORMULA_BUILDING_NUM, false, formula.Rarity, 1)

		slot_buildings[0].FormulaId = proto.Int32(0)
		slot_buildings[0].RemainSeconds = proto.Int32(0)
		tmp_slot_id := slot_buildings[0].GetSlotId()
		n := 0
		for ; n < len(slot_buildings)-1; n++ {
			slot_buildings[n] = slot_buildings[n+1]
		}
		slot_buildings[n].SlotId = proto.Int32(tmp_slot_id)

		cost_diamond = (remain_seconds + (global_id.FormulaSpeedupMakingBuildingCostDiamond_52 - 1)) / global_id.FormulaSpeedupMakingBuildingCostDiamond_52
		this.SubDiamond(cost_diamond, "speedup_making_building", "formula")
	}

	response := &msg_client_message.S2CSpeedupMakeBuildingResult{}
	response.SlotId = proto.Int32(making_list[0])
	this.Send(response)
...
func (this *Player) cancel_making_formula_building(slot_id int32) int32 {
	...
	response.SlotId = proto.Int32(slot_id)
	...
	for i := 0; i < len(f.CostItems); i++ {
		...
		response.ReturnMaterials[i].ItemCfgId = proto.Int32(f.CostItems[i].Id)
		item_num := int32(f.CostItems[i].Num * global_id.CancelMakingFormulaReturnMaterial_19 / 100)
		response.ReturnMaterials[i].ItemNum = proto.Int32(item_num)
