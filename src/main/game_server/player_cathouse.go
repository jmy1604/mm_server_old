...
func (this *dbPlayerCatHouseColumn) Get4RPC(building_id int32) (data *rpc_common.H2H_CatHouseData) {
	...
	data = &rpc_common.H2H_CatHouseData{
		CatIds:        cat_ids,
...
func get_cathouse_table_data(cathouse_cid, level int32) *table_config.XmlCatHouseItem {
...
func (this *Player) if_cathouse_can_create(cathouse_cid int32) bool {
	building := cfg_building_mgr.Map[cathouse_cid]
	if building == nil {
		return false
	}
...
func (this *Player) send_cathouse_info(building_id int32, on_update bool) {
	...
	msg.House.CatHouseId = proto.Int32(building_id)
	msg.House.CatIds, _ = this.db.CatHouses.GetCatIds(building_id)
	msg.House.Level = proto.Int32(level)
	msg.House.Gold = proto.Int32(curr_gold)
	msg.House.NextLevelRemainSeconds = proto.Int32(remain_seconds)
	msg.House.BuildingConfigId = proto.Int32(cathouse_cid)
	msg.House.IsDone = proto.Int32(is_done)
...
func (this *Player) cathouse_add_cat(cat_id int32, building_id int32) int32 {
	...
	response.CatHouseId = proto.Int32(building_id)
	response.CatId = proto.Int32(cat_id)
	response.Gold = proto.Int32(curr_gold)
	this.Send(response)
...
func (this *Player) get_cathouses_info() int32 {
	...
	if all_index == nil || len(all_index) == 0 {
		...
		for _, id := range all_index {
			...
			response.Houses[c].CatHouseId = proto.Int32(id)
			...
			response.Houses[c].Level = proto.Int32(level)
			response.Houses[c].Gold = proto.Int32(curr_gold)
			response.Houses[c].NextLevelRemainSeconds = proto.Int32(remain_seconds)
			response.Houses[c].BuildingConfigId = proto.Int32(cathouse_cid)
			is_done, _ := this.db.CatHouses.GetIsDone(id)
			response.Houses[c].IsDone = proto.Int32(is_done)
			c += 1
...
func (this *Player) cathouse_remove_cat(cat_id, building_id int32) int32 {
	...
	response.CatHouseId = proto.Int32(building_id)
	response.CatId = proto.Int32(cat_id)
	response.Gold = proto.Int32(gold)
	this.Send(response)
...
func (this *Player) cathouse_start_levelup(building_id int32, send_cathouse_info bool) int32 {
	...
	response.CatHouseId = proto.Int32(building_id)
	response.RemainSeconds = proto.Int32(cathouse[next_level-1].Time)
	this.Send(response)
...
func (this *Player) cathouse_speed_levelup(building_id int32) int32 {
	...
	response.CatHouseId = proto.Int32(building_id)
	response.CostDiamond = proto.Int32(cost_diamond)
	this.Send(response)
...
/*func (this *Player) cathouse_sell(building_id int32) int32 {
	res := this.cathouse_can_remove(building_id)
	if res < 0 {
		return res
	}

	res = this.cathouse_remove(building_id)
	if res < 0 {
		return res
	}

	response := &msg_client_message.S2CSellCatHouseResult{}
	response.CatHouseId = proto.Int32(building_id)
	this.Send(response)

	log.Debug("Player[%v] sell cathouse[%v], add coin[%v]", building_id, res)
